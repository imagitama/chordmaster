...
const convertChordLineToEasierChords = chordLine => {
  ...
  const convertedChunks = chunks.map(chunk =>
    /\S/.test(chunk) ? convertToEasierChord(chunk) : chunk
  )

  const convertedChunksWithCorrectWhitespace = convertedChunks.map(
    (convertedChunk, index) => {
...
const SongVerses = ({ artistAndTitle, verses, easierChordsEnabled }) => (
  <VersesListStyled>
    {verses.map(({ verseTitle, lyricsWithChords, copyFrom }) => {
      if (copyFrom) {
        const copiedVerse = verses.find(
          ({ verseTitle }) => verseTitle === copyFrom
        )

        if (!copiedVerse) {
          throw new Error(
            `Failed to copy from verse "${copyFrom}" in song ${artistAndTitle}`
          )
        }
            ...
            {lyricsWithChords.map(([chords, lyric]) => (
              <LyricsWithChordsListItemStyled key={lyric}>
                <strong>
                  {formatChords(
                    easierChordsEnabled
                      ? convertChordLineToEasierChords(chords)
                      : chords
                  )}
                </strong>
                <br />
