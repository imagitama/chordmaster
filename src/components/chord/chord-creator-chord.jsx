        ...
        <StringStatesStyled>
          {stringArray.map((stringNumber, idx) => (
            <StringState
              ...
              onClick={() => onClickTopString(stringNumber)}
            />
          ))}
        </StringStatesStyled>
        {
          <FretNumberStyled
            onClick={onClickFretNumber}
          >{`${firstFretNumber}fr`}</FretNumberStyled>
        }
        {fretNumbers.map(fretNumber => (
          <Fret
            key={fretNumber}
            fingers={frets[fretNumber]}
            onClick={stringNumber =>
              onClickFretString(fretNumber, stringNumber)
            }
          />
        ))}
      </ChordChartStyled>
...
const String = ({ idx, stringNumber, fingerNumber, onClick }) => (
  <StringStyled
    ...
    onClick={() => onClick(stringNumber)}
  >
    {fingerNumber ? (
      <FingerNumberStyled>{fingerNumber}</FingerNumberStyled>
    ) : (
      <EmptyFingerNumber />
    )}
  </StringStyled>
